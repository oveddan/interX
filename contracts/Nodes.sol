...
contract Counter is Node, ITriggerNode {
  ...
  function trigger(
    ...
    int256 newStateVal = _behaviorGraph.getNodeStateVal(tokenId, _nodeId, 'count') + 1;
    _behaviorGraph.setNodeIntStateVal(tokenId, _nodeId, 'count', newStateVal);

    CounterSocketIndeces memory socketIndeces = getSocketIndeces();
    // write the count to the output
    _behaviorGraph.writeToIntOutput(tokenId, _nodeId, socketIndeces.outputCount, newStateVal);
    // trigger the flow edge
    _behaviorGraph.triggerEdge(tokenId, _nodeId, socketIndeces.outputFlow);
  }
}
...
contract Gate is Node, ITriggerNode {
  ...
  function trigger(
    ...
    uint8 toTrigger = _behaviorGraph.getBoolInputVal(tokenId, _nodeId, _triggeringSocketIndex)
      ? socketIndeces.outputGateTrue
      : socketIndeces.outputGateFalse;
    // trigger the flow edge along that socket
    _behaviorGraph.triggerEdge(tokenId, _nodeId, toTrigger);
  }
}
...
contract VariableSet is Node, ITriggerNode {
  ...
  function trigger(
    ...
    string memory variableSocketName = _behaviorGraph.getStringInputVal(tokenId, _nodeId, socketIndeces.variableName);

    // determine what type of value is stored
    ValueType _inputValueType = _behaviorGraph.getInputValueType(tokenId, _nodeId);

    // if it is an int variable
    if (_inputValueType == ValueType.Int) {
      _behaviorGraph.setVariable(
        tokenId,
        variableSocketName,
        _behaviorGraph.getIntInputVal(tokenId, _nodeId, socketIndeces.inputVal)
      );
    } else {
      _behaviorGraph.setVariable(
        tokenId,
        variableSocketName,
        _behaviorGraph.getBoolInputVal(tokenId, _nodeId, socketIndeces.inputVal)
      );
    }
  }
}
...
contract Add is Node, IFunctionNode {
  ...
  function execute(IBehaviorGraph _behaviorGraph, uint16 tokenId, uint16 _nodeId) external {
    Int2Out1SocketIndeces memory socketIndeces = getSocketIneces();

    int256 val = _behaviorGraph.getIntInputVal(tokenId, _nodeId, socketIndeces.input1) +
      _behaviorGraph.getIntInputVal(tokenId, _nodeId, socketIndeces.input2);

    _behaviorGraph.writeToIntOutput(tokenId, _nodeId, socketIndeces.result, val);
  }
}
